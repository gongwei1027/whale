...
class Config(object):
    ...
    """配置参数"""
    def __init__(self, dataset, embedding, filename):
        self.model_name = 'TextCNN'
        self.train_path = dataset + filename + '_train.txt'  # 训练集
        self.dev_path = dataset + filename + '_dev.txt'  # 验证集
        self.test_path = dataset + filename + '_test.txt'  # 测试集
        self.class_list = [int(x.strip()) for x in open(
            dataset + filename + '_class.txt').readlines()]  # 类别名单
        ...
        self.num_classes = max(self.class_list) + 1                         # 类别数
